...
//ByteBuffer printBuffer(200); //ligne effacée peut etre rajoute un init()
char charArray[16];
...
void quicfunc0() {
  ...
  putString((char *) "f0p"); putString(numBuffer); putString((char *) "-P");
  uint8ToString(numBuffer, digitalPinToPort(latest_interrupted_pin));
  putString(numBuffer);
  if (start !=1) putString(getPSTR(" no count"));
  putString((char *) "\n");
};
...
void quicfunc1() {
  ...
  uint8ToString(numBuffer, latest_interrupted_pin);
  putString(getPSTR("f1p")); putString(numBuffer); putString((char *) "-P");
  uint8ToString(numBuffer, digitalPinToPort(latest_interrupted_pin));
  putString(numBuffer);
  if (start !=1) putString(getPSTR(" (counting off)"));
  putString((char *) "\n");
};
*/
...
void quicfunc2() {
  ...
  *led_port&=not_led_mask;
  //uint8ToString(numBuffer, PCintPort::s_count); putString(numBuffer);
  *led_port|=led_mask; // 73 micros to get here from above.  Second pulse in "Rigol Timing Example"
  *led_port&=not_led_mask;*/
  putString(getPSTR(" f2: P"));/*
  uint8ToHexString(numBuffer, *portInputRegister(3)); putString(numBuffer);// C port
  putString(getPSTR(" pin:")); uint8ToString(numBuffer, latest_interrupted_pin); putString(numBuffer);
  putString(getPSTR(" c")); uint8ToHexString(numBuffer, PCintPort::curr); putString(numBuffer);
  putString(getPSTR(" l")); uint8ToHexString(numBuffer, PCintPort::s_lastPinView); putString(numBuffer);
  putString(getPSTR(" r")); uint8ToHexString(numBuffer, PCintPort::s_portRisingPins); putString(numBuffer);
  putString(getPSTR(" f")); uint8ToHexString(numBuffer, PCintPort::s_portFallingPins); putString(numBuffer);
  putString(getPSTR(" m")); uint8ToHexString(numBuffer, PCintPort::s_pmask); putString(numBuffer);
  putString(getPSTR(" P")); put(PCintPort::s_PORT); putString("\r\n");
  putString(getPSTR("cp")); uint8ToHexString(numBuffer, PCintPort::s_changedPins); putString(numBuffer);
  putString(getPSTR(" cXORlpv")); uint8ToHexString(numBuffer, PCintPort::s_currXORlastPinView); putString(numBuffer); 
  putString(getPSTR(" rp_nCurr")); uint8ToHexString(numBuffer, PCintPort::s_portRisingPins_nCurr); putString(numBuffer); 
  putString(getPSTR(" fp_nNCurr")); uint8ToHexString(numBuffer, PCintPort::s_portFallingPins_nNCurr); putString(numBuffer);
  */putString("\r\n");
  if (PCintPort::pcint_multi > 0) {
    putString("MULTI!\n"); PCintPort::pcint_multi=0;
  }
  if (PCintPort::PCIFRbug > 0) { putString("ERROR: BUG- PCIFR should be reset!"); PCintPort::PCIFRbug=0; }
  ...
  /*if (PCintPort::s_count > 2) {
    for (uint8_t i=0; i < PCintPort::s_count; i++) {
      uint8ToHexString(numBuffer, PCintPort::s_registers[i]); putString(numBuffer); putString(" ");
    }
  }
  PCintPort::s_count=0;*/
  /*
  if (start == 1) {
    putString(getPSTR("STOP Count off\n"));
    putString(getPSTR("Intr OFF: (")); uint8ToString(numBuffer, tPIN1), putString(numBuffer);
    putString((char *) " "); uint8ToString(numBuffer, tPIN3), putString(numBuffer);
    putString((char *) " "); uint8ToString(numBuffer, tPIN5), putString(numBuffer);
    putString((char *) ")\n");
    ...
    start=1;
    interrupt_count[latest_interrupted_pin]++;
    putString(getPSTR("START! p"));
    uint8ToString(numBuffer, latest_interrupted_pin);
    putString(numBuffer); putString((char *) "-P");
    // MIKE put the REAL PORT HERE
    uint8ToString(numBuffer, digitalPinToPort(latest_interrupted_pin));
    putString(numBuffer); putString((char *) "\n");
    if (! initial) {
...
void setup() {
  ...
  //Serial.println(getCapacity(), DEC);
  //Serial.println("*---------------------------------------*");
  Serial.print("*---*");
  delay(250);
  begintime=millis();
...
void loop() {
  ...
  char outChar;
  // uint8_t bufsize;
  //if (getSize() != 0) { Serial.print("SZ:"); Serial.println (getSize(), DEC); };
  //bufsize=getSize();
  //if (bufsize > 0) { Serial.print("S:"); Serial.println(bufsize); }
  while ((outChar=(char)getdata()) != 0) Serial.print(outChar);
